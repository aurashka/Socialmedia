...
export const parseContent = (text: string, users: Record<string, User>): React.ReactNode[] => {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('#')) {
      ...
      return <a key={index} href={`#/search/${encodeURIComponent(tag)}`} className="text-primary dark:text-gray-100 font-semibold hover:underline">{part}</a>;
    }
    if (part.startsWith('@')) {
      const handle = part.substring(1).toLowerCase();
      const user = usersByHandle[handle];
      if (user) {
        return <a key={index} href={`#/profile/${user.id}`} className="text-primary dark:text-gray-100 font-semibold hover:underline">{part}</a>;
      }
      return <span key={index} className="text-primary dark:text-gray-100 font-semibold">{part}</span>;
    }
    if (part.match(/^(https?:\/\/|www\.)/)) {
      const url = part.startsWith('www.') ? `http://${part}` : part;
      return <a key={index} href={url} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">{part}</a>;
    }
